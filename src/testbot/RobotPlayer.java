...
public strictfp class RobotPlayer {
    ...
    static private boolean spawned = false;
    ...
    public static void run(RobotController rc) throws GameActionException {
        ...
        while (true) {
            ...
            try {
                ...
                if(randNum == null) randNum = new Random(rc.getID());
                if (rc.readSharedArray(0) == rc.getID()) trySpawnDuck(rc);
                if(rc.isSpawned()){
                    // [ Builder Duck Specialty]
                    // int battleRound = rc.getRoundNum();
                    // if (battleRound <= ROUNDS_TO_FIND_CRUMBS) BuilderSetupPhase.runSetupPhase(rc);
                    // else {
                    //     if (rng.nextBoolean()) {
                    ...
                    MapLocation target = new MapLocation(7,7);
                    navigator.march2Target(rc, target);
                }
    ...
    private static void trySpawnDuck(RobotController myRobotDuck) throws GameActionException{
        ...
        if(myRobotDuck.canSpawn(randSpawn) && !spawned) {
